...
func TestConvert(t *testing.T) {
	var testFixtures = []testCase{
		...
		},
		{
			...
			outputType:  "jpg",
			path:        "fixtures/gif",
			outputFile: []string{"fixtures/gif/input1/gopher1.jpg", "fixtures/gif/input2/gopher2.jpg"},
		},
	}

	for _, testFixture := range testFixtures {
		c := converter.New(testFixture.inputType, testFixture.outputType)
		if err := c.ConvertImage(testFixture.path); err != nil {
		...
		for _, file := range testFixture.outputFiles {
			...
			checkFormat(t, file, testFixture.outputType)
		}
	}
}

func checkFormat(t *testing.T, path string, fileType string) bool {
	t.Helper()
